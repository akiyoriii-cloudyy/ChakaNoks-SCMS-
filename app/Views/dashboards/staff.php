    ...
    <header class="header">
        ...
        <nav class="tabs">
            <?php if (($me['role'] ?? '') !== 'inventory_staff'): ?>
                <a class="tab" href="<?= base_url('dashboard') ?>">Dashboard</a>
                <a class="tab active" href="<?= base_url('inventory') ?>">Inventory</a>
                <a class="tab" href="<?= base_url('purchases') ?>">Purchases</a>
                <a class="tab" href="<?= base_url('settings') ?>">Settings</a>
            <?php else: ?>
        ...
        <div class="user-strip" style="display:flex;justify-content:flex-end;align-items:center;gap:15px;padding:10px;">
            <div style="background: linear-gradient(135deg, #2ecc71, #27ae60); color: white; padding: 10px 20px; border-radius: 25px; font-weight: bold; box-shadow: 0px 4px 10px rgba(0,0,0,0.2);">
    ...
    <section class="card">
        ...
        <div class="table-wrap">
            <table class="table" id="invTable">
                ...
                <tbody id="invBody"><!-- IMPORTANT: id needed by JS -->
                <?php foreach ($items as $row): ?>
                    <tr
                        ...
                        data-stockqty="<?= esc($row['stock_qty']) ?>"
                        data-unit="<?= esc($row['unit'] ?? '') ?>"
                        data-expiry="<?= esc($row['expiry'] ?? '') ?>"
                    >
                        <!-- Item -->
                        <td>
                        ...
                        <td>
                            <div class="cell-title">
                                <?= isset($row['stock_qty']) ? esc($row['stock_qty']) . ' ' . (isset($row['unit']) ? esc($row['unit']) : '') : 'N/A' ?>
                            </div>
                        ...
                        <td class="right">
                            <button class="link viewBtn"
                                ...
                                data-branch="<?= esc($row['branch']) ?>"
                                data-stock="<?= isset($row['stock_qty']) ? esc($row['stock_qty']) : '0' ?>"
...
<div class="modal" id="viewItemModal" hidden>
    ...
    <div class="modal-card">
        ...
        <div class="view-body">
            ...
            <p><b>Expiry Status:</b> <span id="viewExpiryStatus"></span></p>
...
<!-- Initial data for JS -->
<script id="initial-items" type="application/json">
<?= json_encode($items) ?>
</script>

<!-- Single, cache-busted JS include -->
